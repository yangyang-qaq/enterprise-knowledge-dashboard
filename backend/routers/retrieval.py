...
RETRIEVAL_CONFIG_KEYS = {
    ...
    'ENABLE_GRAPH_RETRIEVAL': 'rag.enable_graph_retrieval',
    ...
    'GRAPH_BONUS': 'rag.graph_bonus',
    'GRAPH_CHUNK_BUDGET': 'rag.graph_chunk_budget',
    'GRAPH_EXTRACT_CONCURRENCY': 'graph.extract_concurrency',
    'GRAPH_HOPS': 'rag.graph_hops',
    'GRAPH_HUB_DEGREE_CAP': 'rag.graph_hub_degree_cap',
    'GRAPH_MERGE_MODE': 'rag.graph_merge_mode',
...
async def get_rag_config(request: Request, user=Depends(get_admin_user)):
    ...
    return {
        ...
        'GRAPH_CHUNK_BUDGET': config.GRAPH_CHUNK_BUDGET,
        'GRAPH_MERGE_MODE': config.GRAPH_MERGE_MODE,
        'GRAPH_BONUS': config.GRAPH_BONUS,
        'GRAPH_HUB_DEGREE_CAP': config.GRAPH_HUB_DEGREE_CAP,
...
class ConfigForm(BaseModel):
    ...
    GRAPH_CHUNK_BUDGET: int | None = None
    GRAPH_MERGE_MODE: str | None = None
    GRAPH_HUB_DEGREE_CAP: int | None = None
...
async def update_rag_config(request: Request, form_data: ConfigForm, user=Depends(get_admin_user)):
    ...
    config.ENABLE_GRAPH_RETRIEVAL = (
        form_data.ENABLE_GRAPH_RETRIEVAL
        if form_data.ENABLE_GRAPH_RETRIEVAL is not None
        else config.ENABLE_GRAPH_RETRIEVAL
    )
    config.GRAPH_HOPS = form_data.GRAPH_HOPS if form_data.GRAPH_HOPS is not None else config.GRAPH_HOPS
    config.GRAPH_CHUNK_BUDGET = (
        form_data.GRAPH_CHUNK_BUDGET if form_data.GRAPH_CHUNK_BUDGET is not None else config.GRAPH_CHUNK_BUDGET
    )
    config.GRAPH_MERGE_MODE = (
        form_data.GRAPH_MERGE_MODE if form_data.GRAPH_MERGE_MODE is not None else config.GRAPH_MERGE_MODE
    )
    config.GRAPH_HUB_DEGREE_CAP = (
        form_data.GRAPH_HUB_DEGREE_CAP
        if form_data.GRAPH_HUB_DEGREE_CAP is not None
        else config.GRAPH_HUB_DEGREE_CAP
    )
    config.GRAPH_BONUS = form_data.GRAPH_BONUS if form_data.GRAPH_BONUS is not None else config.GRAPH_BONUS
