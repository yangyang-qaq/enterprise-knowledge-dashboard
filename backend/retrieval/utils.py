...
from open_webui.retrieval.graph_retrieval import maybe_expand_query_result_with_graph
...
async def query_collection(
    ...
    """Query the vector store, optionally expanding the result through the graph.

    ``knowledge_id`` is the *knowledge base* id, not a file id: the graph is built
    per KB. It is keyword-only in spirit -- every existing caller leaves it unset
    and gets the previous behaviour byte for byte.

    ``use_graph`` overrides ``rag.enable_graph_retrieval`` for this call only
    (``None`` = follow config). That exists so the A/B eval drives the same
    function, same ``k``, same reranker, and the only difference is this bool --
    otherwise the comparison measures the harness rather than the feature.
    """
    config = await Config.get_many(
        ...
        'rag.enable_graph_retrieval',
        'rag.graph_hops',
        'rag.graph_chunk_budget',
        'rag.graph_merge_mode',
        'rag.graph_hub_degree_cap',
        'rag.graph_bonus',
    )
    ...
    if request and config.get('rag.enable_hybrid_search'):
        try:
            ...
            # Assigned rather than returned. Graph expansion runs afterwards on
            # whichever branch produced this result, and must not be able to
            # discard a perfectly good hybrid result if it fails.
    ...
    graph_enabled = (
        config.get('rag.enable_graph_retrieval') if use_graph is None else use_graph
    )
    graph_query = next((q for q in queries if q), '')
    result = await maybe_expand_query_result_with_graph(
        result,
        ...
        hybrid_enabled=bool(config.get('rag.enable_hybrid_search')),
        hops=config.get('rag.graph_hops'),
        budget=config.get('rag.graph_chunk_budget'),
        merge_mode=config.get('rag.graph_merge_mode'),
        hub_cap=config.get('rag.graph_hub_degree_cap'),
        graph_bonus=config.get('rag.graph_bonus'),
    )
...
async def get_sources_from_items(
    ...
    for item in items:
        query_result = None
        collection_names = []
        # Set only when the item resolves to a whole knowledge base. The graph is
        # built per KB, so single-file collections (`file-{id}`), legacy KBs,
        # `context == 'full'`, external KBs and `item['docs']` all leave this
        # None and therefore skip graph expansion entirely.
        graph_kb_id = None
