...
# Phase 12: Graph-Enhanced RAG (multi-hop expansion recall).
# Expansion reuses the reranker to keep every candidate in one score space, so
# it is inert unless hybrid search and a reranker are both configured -- see
...
GRAPH_CHUNK_BUDGET = int(os.getenv('GRAPH_CHUNK_BUDGET', '0'))  # 0 = auto: max(2, k // 3)
GRAPH_MERGE_MODE = os.getenv('GRAPH_MERGE_MODE', 'append')  # 'rerank' | 'append'
GRAPH_HUB_DEGREE_CAP = int(os.getenv('GRAPH_HUB_DEGREE_CAP', '20'))
# Additive nudge for graph-recalled chunks, applied on top of their own rerank
# score. Tunable because the "obviously negligible" 0.05 is not: bge-reranker-base
# returns sigmoid scores in [0, 1] whose top results cluster within ~0.01 of each
# other, so 0.05 does not break ties -- it promotes. Measured on the 4-doc demo KB
# (16 questions, K=3, file-level recall): the default 0.05 made the graph arm
# *worse* than no graph at all (overall Δ -0.073, cross-doc Δ -0.146), because
# graph chunks outranked base chunks and pushed them past position K. Hence the
# 'append' default above. Set 0 to disable the bonus entirely.
GRAPH_BONUS = float(os.getenv('GRAPH_BONUS', '0.05'))
GRAPH_EXTRACT_CONCURRENCY = int(os.getenv('GRAPH_EXTRACT_CONCURRENCY', '4'))
GRAPH_EXTRACT_MODEL = os.getenv('GRAPH_EXTRACT_MODEL', '')  # empty = reuse EVAL_MODEL
...
DEFAULT_CONFIG = {
    ...
    'rag.enable_graph_retrieval': ENABLE_GRAPH_RETRIEVAL,
    'rag.graph_hops': GRAPH_HOPS,
    'rag.graph_chunk_budget': GRAPH_CHUNK_BUDGET,
    'rag.graph_merge_mode': GRAPH_MERGE_MODE,
    'rag.graph_hub_degree_cap': GRAPH_HUB_DEGREE_CAP,
    'rag.graph_bonus': GRAPH_BONUS,
    'graph.extract_concurrency': GRAPH_EXTRACT_CONCURRENCY,
    'graph.extract_model': GRAPH_EXTRACT_MODEL,
